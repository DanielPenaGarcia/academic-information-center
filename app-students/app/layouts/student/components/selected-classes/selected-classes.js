document.addEventListener("DOMContentLoaded", function () {
  ...
  function removeClass(classInfo) {
    ...
    enrolledClasses = enrolledClasses.filter(
      (classItem) => classItem !== classInfo
    );

    // Send updated list back to parent
    window.parent.postMessage(
      { type: "REMOVE_ENROLLED_CLASSES", class: classInfo },
      "*"
    );
  }
  ...
  document.getElementById("enroll-button")?.addEventListener("click", () => {
    if (enrolledClasses.length > 0) {
      alert(
        `You have enrolled in: ${enrolledClasses
          .map((classItem) => classItem.name)
          .join(", ")}`
      );
      enrolledClasses = []; // Clear the list after enrollment
      window.parent.postMessage(
        { type: "UPDATE_ENROLLED_CLASSES", classes: enrolledClasses },
        "*"
      );
    } else {
